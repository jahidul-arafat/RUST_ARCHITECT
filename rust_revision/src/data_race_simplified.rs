...
fn error_scenario_cannot_borrow_data_as_immutable_because_it_is_also_borrowed_as_mutable(){
    ...
    let ref1 = &mut data;
    let ref2 = &data; // error: cannot borrow `data` as immutable, as it is already borrowed as mutable
...
fn error_scenario_cant_borrow_data_as_mutable_more_than_once_at_a_time(){
    let mut data = vec ! [1, 2, 3];
    let ref1 = &mut data;
    let ref2 = &mut data; // error: cant_borrow_data_as_mutable_more_than_once_at_a_time
